...
const TICKER = [
  "SENSEX up 450 pts to 82,470",
  "Rupee strengthens to 83.15 against USD",
  "Gold prices rise ₹500 to ₹72,300/10g",
  "Petrol prices remain unchanged",
  "India's GDP growth forecast raised to 7.2%",
];

const MARKETS = [
  { name: "NIFTY 50", value: "25,102.40", change: "+0.62%", up: true },
  { name: "RELIANCE", value: "₹2,912.05", change: "+1.18%", up: true },
  { name: "TCS", value: "₹3,860.90", change: "-0.34%", up: false },
  { name: "HDFC BANK", value: "₹1,634.80", change: "+0.44%", up: true },
  { name: "INFY", value: "₹1,466.10", change: "-0.12%", up: false },
];
    ...
    <header className="sticky top-0 z-50 bg-background">
      <div className="bg-ink text-background">
        <div className="mx-auto grid max-w-[1200px] grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-1.5 text-[11px]">
          ...
          <p className="min-w-0 truncate opacity-90">
            UPDATE: Mumbai rainfall breaks 50-year record, NDRF teams deployed
          </p>
        </div>
      </div>
        ...
        <div className="mx-auto grid max-w-[1200px] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 lg:flex lg:justify-between">
          <a href="/" className="flex min-w-0 items-center gap-2">
            ...
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-[9px] tracking-[0.28em] text-muted-foreground uppercase">
              ...
              {[0, 1].map((dup) => (
                <span key={dup} className="flex">
                  {TICKER.map((t) => (
                    <span
                      key={t}
                      className="px-6 text-[11px] text-muted-foreground"
                    >
                      {t}
                    </span>
                  ))}
      ...
      <div className="border-b border-border">
        <div className="mx-auto flex max-w-[1200px] gap-5 overflow-x-auto px-4 py-1.5 text-[11px]">
          {MARKETS.map((m) => (
            <div key={m.name} className="flex shrink-0 items-center gap-1.5">
