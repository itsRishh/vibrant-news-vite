...
const POSITION_OPTIONS = [
  ...
  { value: 5, label: "Sub hero 4" },
  { value: 6, label: "Ad / no-visual slot" },
  { value: 7, label: "No visual 1" },
  { value: 8, label: "No visual 2" },
  { value: 9, label: "No visual 3" },
  { value: 10, label: "No visual 4" },
  { value: 11, label: "No visual 5" },
] as const;
...
function AdminRegionalNews() {
  ...
  const [badge, setBadge] = useState("REGIONAL");
  ...
  async function onPositionChange(articleId: string, nextPosition: number) {
    ...
    try {
      await moveRegionalNews({
        id: articleId as Id<"regionalNews">,
    ...
    if (!title.trim() || !category || !excerpt.trim() || (selectedPositionRequiresMedia && !media)) {
      setError(
        selectedPositionRequiresMedia
          ...
          : "Headline, category, and excerpt are required for this slot.",
      );
      return;
    }
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[1250px] bg-background px-4 py-10 text-foreground sm:px-6">
      <div className="mb-8 border-b border-border pb-5">
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight">Publish Regional News</h1>
        <p className="mt-2 text-sm text-muted-foreground">Manage the hero, sub-hero cards, ad space, and six no-visual regional slots.</p>
      </div>
        ...
        <div className="flex w-full flex-col gap-5 lg:w-[50%]">
          <label className="block w-full text-sm font-bold">
            ...
            <span className="mt-1 block text-xs font-normal text-muted-foreground">
              Positions 1-6 use media; 7-11 are text-only slots. Position 6 is the ad placeholder.
            </span>
          </label>
          ...
          <label className="block w-full text-sm font-bold">
            ...
            <input
              ...
              placeholder="Regional update headline"
            />
          </label>
          ...
          <label className="block w-full text-sm font-bold">
            Excerpt
            <textarea value={excerpt} onChange={(event) => setExcerpt(event.target.value)} className="mt-2 min-h-28 w-full border border-border px-3 py-2 font-normal" placeholder="Short summary for the regional card." />
          </label>
        ...
        <div className="flex w-full flex-col gap-5 lg:w-[50%]">
          {selectedPositionRequiresMedia ? (
            ...
            <div className="flex min-h-32 items-center justify-center border border-dashed border-border bg-muted/20 p-4 text-sm text-muted-foreground">
              This slot is intentionally text-only.
            </div>
          )}
